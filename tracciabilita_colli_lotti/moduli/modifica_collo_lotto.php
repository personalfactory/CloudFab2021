<?php  
	...
	include("../../tracciabilita_colli_lotti/sql/query.php");
	  ...
	  if (isset($_POST["codiceCollo"])) {
			  ...
			  if (isset($_POST["info1collo"])) { $info1collo = $_POST["info1collo"]; echo $info1collo;}
		  ...
		  if ($codiceCollo != ""){
				...
				updateCollo($idCollo,$codiceCollo, $dataCollo,$altezzaCollo,$larghezzaCollo,$profonditaCollo,$pesoCollo,$info1collo,$info2collo,$info3collo,$info4collo,$info5collo);
 	 
 			  	//Registrazione Codice Collo Lotti 
 				$codiciLottoDisabilitare = $_POST["codiciLottoDisabilitare"];	
		    	for ($i = 0; $i < count($codiciLottoDisabilitare); ++$i) { 
			   		disabilitaColloLotti($codiciLottoDisabilitare[$i]); 
		  			} 
				
			   	//Inserimenti Codici Lotto
			  $codiciLottoInserire = $_POST["codiciLottoInserire"];
 				for ($i = 0; $i < count($codiciLottoInserire); ++$i) {  
 					insertColloLotti($idCollo, $codiciLottoInserire[$i]); 
 		 		}
		} 
	  }
