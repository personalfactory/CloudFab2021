...
<html>
    ...
    <head>
		...
		function salva(){
			...
			if(document.getElementById('respEvasione').selectedOptions[0].value<=0){//document.getElementById('respOrdine').value===""){
			   
				alert("<?php echo $msgErroreOrdine2; ?>");
			   
			} else {
				registraEvasioneOrdine();
				
			} 
			 
		}
			
		function registraEvasioneOrdine() {
		   
				var richiesta = new XMLHttpRequest();  
			var dati_evasione = [];
			dati_evasione.push("<?php echo $id; ?>");
			dati_evasione.push(document.getElementById('dataEvasione').innerHTML);
			dati_evasione.push(document.getElementById('respEvasione').selectedOptions[0].text); //document.getElementById('respOrdine').value);  
			dati_evasione.push(document.getElementById('noteEvasione').value);  
			
			
				richiesta.onload = function() { 
						var res = this.responseText;  
						alert("<?php echo $msgOrdineEvaso ?>"); 
 						location.href = "ordini_produzione.php";
					};  
				richiesta.open("get", "moduli/registra_evasione_ordine.php?dati_evasione="+JSON.stringify(dati_evasione), true); 
				richiesta.send();
	...
	<body onload="aggiungiRiga()">
        ...
        <table width="100%" class="table2" id= "dati_odine">
			...
			<tr>
				<td class="cella3" colspan="5" style="font-size: 14px;" id="rep";><?php echo $titleDettagliEvasione; ?></td>
			</tr> 
			 
			<tr> 
				<th class="cella2" colspan="1" style="font-size: 12px;"> <label><?php echo $titleDettagliEvasioneData; ?></label></th> 
				<th class="cella1" colspan="4" id="dataEvasione" style="font-size: 14px; font-weight:normal; "> <?php date_default_timezone_set('UTC'); echo date('Y-m-d H:i:s'); ?></th>
			</tr> 
			<tr> 
				<th class="cella2" colspan="1" style="font-size: 12px; "><?php echo $titleDettagliEvasioneResponsabile; ?> </th>
				<th  class="cella1 "  colspan="4">
					<select class="select" id="respEvasione" style="font-size:12pt; font-weight:normal; height:20px;width:300px;">  
			...
			<tr> 
				<th class="cella2" colspan="1" style="font-size: 12px; width:100%; "><?php echo $titleDettagliEvasioneNote; ?> </th>
                <th  class="cella1" colspan="4">
					<textarea id="noteEvasione" name="noteEvazione" style="font-size:12pt; font-weight:normal; height:100px;width:800px; float: right;text-align:right"> </textarea> </td> 
			</tr> 
			
		</table>

	<input class = "button2" type='button' style="font-size:18pt;height:420px;width:200px; float: right;" value="<?php echo $titleButtonEvadi; ?>" onClick="salva()" />
        </div><!--mainContainer--> 
    </body>
</html>