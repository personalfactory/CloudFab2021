...
//            document.forms["ModificaSoluzioneInsacco"].ScrollX.value = document.body.scrollLeft;
//            document.forms["ModificaSoluzioneInsacco"].value = document.body.scrollTop;
//        }
//        function ResetScrollPosition() {
//            var hidx, hidy;
//            hidx = document.forms["ModificaSoluzioneInsacco"].ScrollX;
//            hidy = document.forms["ModificaSoluzioneInsacco"].ScrollY;
...
//        }
        function mostraOrigami4() {
            ...
            for (i = 0; i < document.getElementsByName('chart_div').length; i++) {
                ...
                document.getElementsByName('chart_vel')[i].style.display = "none";
            }
            document.getElementById("tabContainer").style.height = "1170px";
        }
        function nascondiOrigami4() {
            ...
            for (i = 0; i < document.getElementsByName('chart_div').length; i++) {
                ...
                document.getElementsByName('chart_vel')[i].style.display = "block";
            }
        }
        function Salva() {

            document.forms["ModificaSoluzioneInsacco"].action = "manage_val_par_sacco1.php";
        ...
        function AggiornaConfigurazione() {
            ...
            document.forms["ModificaSoluzioneInsacco"].action = "manage_val_par_sacco.php";
            document.forms["ModificaSoluzioneInsacco"].submit();

        }
//        function Conferma() {
//
//            alert('Sicuro di voler effettuare la copia?');
//
//        }
    ...
    <body>
        <div id="mainContainer">
            ...
            } else if (isSet($_POST['IdCategoria']) AND isSet($_POST['NumSacchetti'])) {
            ...
            $SaccoOrigine = 0;
            $SaccoDestinazione = 0;
            if (isSet($_POST['SaccoOrigine']))
                $SaccoOrigine = $_POST['SaccoOrigine'];
            if (isSet($_POST['SaccoDestinazione']))
            ...
            <div id="container" style=" width:100%; margin:15px auto;">
                <form id="ModificaSoluzioneInsacco" name="ModificaSoluzioneInsacco" method="POST">
                    ...
                    <table width="100%">
                        ...
                        <tr >
                            <td class="cella2" ><?php echo $filtroQtaMiscelaStandard ?></td>
                            <td class="cella1" ><?php echo number_format($valDefaultPesoMiscela, "0", ",", ".") . " " . $filtrogBreve ?>&ensp;&ensp;&ensp;<?php echo $titleQtaMiscelaStandard; ?></td>
                        </tr> 
                    </table>
                    <table width="100%">
                        <tr>
                            <!-- ###########################################################-->
                            <!-- FORM PER COPIARE I PARAMETRI DA UN SACCO AD UN ALTRO-->
                            <!-- ###########################################################-->

                            <td class="cella2" style="text-align: left" >
                                &nbsp;<span > <?php echo $filtroCopiaParSacco ?></span>

                                <select name="SaccoOrigine" id="SaccoOrigine">
                                    <option value="<?php echo $SaccoOrigine ?>" selected=""><?php echo $SaccoOrigine ?></option>
<?php
for ($s = 1; $s <= $NumSacchetti; $s++) {
    ?>
                                ...
                                &nbsp;<span style="border:none;padding-right:5px;"> <?php echo $filtroCopiaParSacco2 ?></span>
                                <select name="SaccoDestinazione" id="SaccoDestinazione" >
                                    <option value="<?php echo $SaccoDestinazione ?>" selected=""><?php echo $SaccoDestinazione ?></option>
<?php
for ($t = 1; $t <= $NumSacchetti; $t++) {
    ?>
                                ...
                                &nbsp;&nbsp;
                                <input type="button" onClick="AggiornaConfigurazione()" value="<?php echo $valueButtonCopia ?>" /></td>
                            ...
                            <td class="cella2" style="text-align: right " >
                                ...
                                <!--<input type="button" onClick="javascript:AggiornaConfigurazione()" value="<?php echo $valueButtonAggiorna ?>" />-->
                                <input type="button" onClick="javascript:Salva()" value="<?php echo $valueButtonVerifica ?>" title="<?php echo $titleVerificaPrimaDiSalvare ?>"/></td>
                        </tr>
                    </table>
...
//                            $aliquotaConf['nomePar'][3] = $row['nome_variabile'];
    ...
    ###################################################################
    ############# COPIA I VALORI DI UN SACCO IN UN ALTRO ##############
    ###################################################################
    if ($SaccoOrigine != $SaccoDestinazione AND $sacco == $SaccoDestinazione) {
        $s = 0;
        $w = 0;
        $p = 0;
        $a = 0;
        ...
        $valoriSaccoOrigine = selectValoriSaccoBySoluzioneCat($IdCategoria, $NumSacchetti, $SaccoOrigine);
        while ($row = mysql_fetch_array($valoriSaccoOrigine)) {

            switch ($row['tipo']) {
                case '1':
                    $arraySacco['valore'][$s] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $s++;
                    break;

                case '2':
                    $arrayVelocita['valore'][$w] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $w++;
                    break;

                case '3':
                    $arrayPesatura['valore'][$p] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $p++;
                    break;

                case '4':
                    $arrayTempiApertura['valore'][$a] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $a++;
                    break;

                case '5':
                    $arrayTempiChiusura['valore'][$c] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $c++;
                    break;

                case '1-O4':
                    $arraySaccoO4['valore'][$s4] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $s4++;
                    break;

                case '3-O4':
                    $arrayPesaturaO4['valore'][$p4] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $p4++;
                    break;

                case '4-O4':
                    $arrayTempiAperturaO4['valore'][$a4] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $a4++;
                    break;
                case '5-O4':
                    $arrayTempiChiusuraO4['valore'][$c4] = $_POST['Valore' . $row["id_val_par_sac"]];
                    $c4++;
                    break;
    ...
    ?>
                        ...
                        <div id="tabContainer">
                            <div class="content">
                                ...
                                <table width="100%" >
                                    <tr>
                                        <td class="cella3" colspan="4" style="font-size: 20px"><?php echo $filtroConfezionamento . " " . $filtroSac . " " . $sacco ?>&ensp;&ensp;&ensp;<?php echo number_format($pesoSaccoDefault, "0", ",", ".") . $filtrogBreve ?></td>
                                    </tr>
                                    <tr>
                                        <td class="cella1" width="10%"><?php echo $filtroId ?></td>
                                        <td class="cella1" width="20%"><?php echo $filtroPar ?></td>
                                        <td class="cella1" width="30%"><?php echo $filtroValore ?></td>
                                        <td class="cella1" width="40%"></td>
                    ...
                    for ($j = 0; $j < count($arraySacco['idPar']); $j++) {
                        ?>
                                        <tr>
                                            ...
                                            <td class="cella1" ><?php echo $arraySacco['nomeVis'][$j] ?></td>
                                            <td class="cella1" >
                                                <!--<input type="text" style="width:80px" name="Valore<?php echo $arraySacco['idVal'][$j]; ?>" value="<?php echo($arraySacco['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo ($arraySacco['uniMis'][$j]); ?></td>-->
                                                <textarea rows="1" cols=20" name="Valore<?php echo $arraySacco['idVal'][$j]; ?>" value="<?php echo($arraySacco['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"><?php echo($arraySacco['valore'][$j]); ?></textarea><?php echo ($arraySacco['uniMis'][$j]); ?></td>
                                            <td class="cella1" ></td>
                                        </tr>
    <?php }
    ?>  
                                    <tr>
                                        <td class="cella1" ><?php echo $sbloccoScarico['idPar'][0] ?></td>
                                        <td class="cella1" ><?php echo $sbloccoScarico['nomePar'][0] ?></td>
                                        <td class="cella1" ><?php echo $sbloccoScarico['valore'][0] . " " . $filtrogBreve; ?></td>
                                        <td class="cella1" ></td>
                                    </tr>
                                </table>
                                <table>
                                    <tr>
                                        <td class="dataRigWhite" colspan="9"><?php echo $filtroVelocitaMiscelatore ?> </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="cella2" width="3%"><?php echo $filtroId ?></td>
                                        <td class="cella2" width="15%" ><?php echo $filtroPar ?></td>
                                        <td class="cella2" colspan="2" width="8%" ><?php echo $filtroPercConfezionamento ?></td>   
                                        <td class="cella2" colspan="2" width="10%" ><?php echo $filtroPesoEffettivo ?></td> 
    ...
    for ($j = 0; $j < count($arrayVelocita['idPar']); $j++) {
        ?>
                                        <tr>
                                            ...
                                            <td class="cella2" ><?php echo $arrayVelocita['nomeVis'][$j] ?></td>
                                            <td class="cella2" >
                                                <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayVelocita['idVal'][$j]; ?>" value="<?php echo($arrayVelocita['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo ( $arrayVelocita['uniMis'][$j]); ?></nobr></td>

        <?php
        if ($t > 0) {
            $valPeso = ($pesoSaccoMenoTubo - $aliquotaConf['valore'][$t] * $pesoSaccoMenoTubo / 100);
            $valPeso1 = ($pesoSaccoMenoTubo - $aliquotaConf['valore'][$t + 1] * $pesoSaccoMenoTubo / 100);
            ?>
                                                <td class="cella2" ><?php echo $aliquotaConf['idPar'][$t] ?></td>
                                                <td class="cella2" ><?php echo $aliquotaConf['nomePar'][$t] ?></td>

                                                <td class="cella2" ><?php echo $aliquotaConf['valore'][$t + 1] . $filtroPerc ?></td>

                                                <td class="cella2" ><?php echo $aliquotaConf['valore'][$t] . $filtroPerc ?></td>
                                                <td class="cella2" title="<?php echo $titleValPesoVelocita ?> "><?php echo number_format($valPeso1, "0", ",", ".") . " " . $filtrogBreve ?></td>
                                                <td class="cella2" title="<?php echo $titleValPesoVelocita ?> "><?php echo number_format($valPeso, "0", ",", ".") . " " . $filtrogBreve ?></td>
            ...
            $valPesoF = $pesoSaccoMenoTubo - $arrayPesatura['valore'][0];
            ?>
                                                <!-- INIZIO PESATURA FINE-->
                                                <td class="cella2" ><?php echo $arrayPesatura['idPar'][0] ?></td>
                                                <td class="cella2" ><?php echo $aliquotaConf['nomePar'][4] ?></td>
                                                <td class="cella2" ><?php echo $aliquotaConf['valore'][1] . $filtroPerc ?></td>

                                                <td class="cella2" ><?php echo "0" . $filtroPerc ?></td>
                                                <td class="cella2" title="<?php echo $titleValPesoVelocita ?> "><?php echo number_format($valPeso, "0", ",", ".") . " " . $filtrogBreve ?></td>
                                                <td class="cella2" title="<?php echo $titleValPesoUltimoVelocita ?> "><?php echo number_format($valPesoF, "0", ",", ".") . " " . $arrayPesatura['uniMis'][0] ?></td>
            <?php
        }
        $t--;
    }
    ?>  
                                ...
                                <table>
                                    ...
                                    <tr>
                                        <td class="cella1" width="2%"><?php echo $filtroId ?></td>
                                        <td class="cella1" width="13%"><?php echo $filtroPar ?></td>
                                        <td class="cella1" width="10%" ><?php echo $filtroPesoMancante ?></td>
                                        <td class="cella1" width="10%" ><?php echo $filtroPesoEffettivo ?></td>
                                        <td class="cella2" width="2%"><?php echo $filtroId ?></td>
                                        <td class="cella2" width="13%"><?php echo $filtroPar ?></td>
                                    ...
                                    for ($j = 0; $j < count($arrayPesatura['idPar']); $j++) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="cella1" ><?php echo $arrayPesatura['nomeVis'][$j] ?></td>
                                            <td class="cella1" >
                                                <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayPesatura['idVal'][$j]; ?>" value="<?php echo($arrayPesatura['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayPesatura['uniMis'][$j]); ?></nobr></td>

        <?php
//                                         
        $valPeso = $pesoSaccoDefault - $arraySacco['valore'][0] - $arrayPesatura['valore'][$j];
        ?>
                                            <td class="cella1" title="<?php echo $pesoSaccoDefault . " - " . $arraySacco['nomeVis'][0] . " - " . $arrayPesatura['nomeVis'][$j] ?>" ><?php echo number_format($valPeso, "0", ",", ".") . " " . $arrayPesatura['uniMis'][$j] ?></td>


                                            <td class="cella2" ><?php echo $arrayTempiApertura['idPar'][$j] ?></td>
                                            <td class="cella2" ><?php echo $arrayTempiApertura['nomeVis'][$j] ?></td>
                                            <td class="cella2" >
                                                <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayTempiApertura['idVal'][$j]; ?>" value="<?php echo($arrayTempiApertura['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayTempiApertura['uniMis'][$j]); ?></nobr></td>

                                            <td class="cella2" ><?php echo $arrayTempiChiusura['idPar'][$j] ?></td>
                                            <td class="cella2" ><?php echo $arrayTempiChiusura['nomeVis'][$j] ?></td>
                                            <td class="cella2" >
                                                <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayTempiChiusura['idVal'][$j]; ?>" value="<?php echo($arrayTempiChiusura['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayTempiChiusura['uniMis'][$j]); ?></nobr></td>
                                        </tr>
    <?php }
    ?>  
                                </table>
                                <div style="width:100%;">
                                    <input type="button" name="buttonMostraOrigami" style="border:none;" value="<?php echo $valueMostraOrigami4 ?>" onClick="mostraOrigami4();" title="<?php echo $titleMostraAltreInfoProd ?>"></input>
                                    <input type="button" name="buttonNascondiOrigami" style="border:none;display:none;" value="<?php echo $valueNascondiOrigami4 ?>" onClick="nascondiOrigami4();" title="<?php echo $titleNascondiAltreInfoProd ?>"></input>
...
//                    echo "count par sacco :".count($arraySaccoO4['idPar']);
        ?>
                                            <tr>
                                                ...
                                                <td class="cella1" ><?php echo $arraySaccoO4['nomeVis'][$j] ?></td>
                                                <td class="cella1" >
                                                    <nobr><input type="text" style="width:80px" name="Valore<?php echo $arraySaccoO4['idVal'][$j]; ?>" value="<?php echo($arraySaccoO4['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arraySaccoO4['uniMis'][$j]); ?></nobr></td>
                                                <td class="cella1" ></td>
                                            </tr>
    <?php }
    ?>  
    ...
    for ($j = 0; $j < count($arrayPesaturaO4['idPar']); $j++) {
        ?>
                                            <tr>
                                                <td class="cella1" ><?php echo $arrayPesaturaO4['idPar'][$j] ?></td>
                                                <td class="cella1" ><?php echo $arrayPesaturaO4['nomeVis'][$j] ?></td>
                                                <td class="cella1" >
                                                    <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayPesaturaO4['idVal'][$j]; ?>" value="<?php echo($arrayPesaturaO4['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayPesaturaO4['uniMis'][$j]); ?></nobr></td>

        <?php
        $valPesoO4 = $pesoSaccoDefault - $arraySaccoO4['valore'][0] - $arrayPesaturaO4['valore'][$j];
        ?>
                                                <td class="cella1" title="<?php echo $pesoSaccoDefault . " - " . $arraySacco['nomeVis'][0] . " - " . $arrayPesaturaO4['nomeVis'][$j] ?>" ><?php echo number_format($valPesoO4, "0", ",", ".") . " " . $arrayPesaturaO4['uniMis'][$j] ?></td>
                                                <td class="cella2" ><?php echo $arrayTempiAperturaO4['idPar'][$j] ?></td>
                                                <td class="cella2" ><?php echo $arrayTempiAperturaO4['nomeVis'][$j] ?></td>
                                                <td class="cella2" >
                                                    <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayTempiAperturaO4['idVal'][$j]; ?>" value="<?php echo($arrayTempiAperturaO4['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayTempiAperturaO4['uniMis'][$j]); ?></nobr></td>

                                                <td class="cella2" ><?php echo $arrayTempiChiusuraO4['idPar'][$j] ?></td>
                                                <td class="cella2" ><?php echo $arrayTempiChiusuraO4['nomeVis'][$j] ?></td>
                                                <td class="cella2" >
                                                    <nobr><input type="text" style="width:80px" name="Valore<?php echo $arrayTempiChiusuraO4['idVal'][$j]; ?>" value="<?php echo($arrayTempiChiusuraO4['valore'][$j]); ?>" onChange="AggiornaConfigurazione()"/><?php echo($arrayTempiChiusuraO4['uniMis'][$j]); ?></nobr></td>
                                            </tr>
    <?php }
    ?>  
                                    </table>
