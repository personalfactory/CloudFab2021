...
<html>
    ...
    $sqlProdotti = findProdottiBsNew($strUtentiAziende);
    ...
    if ($ToDo == "ModificaProdotto" AND $_GET['IdProdotto'] > 0) {
        ...
        $sql = findBSProdottoById($_GET['IdProdotto']);
        while ($row = mysql_fetch_array($sql)) {
            ...
            $SchedaTecnica = $row['link_scheda_tecnica'];
...
<?php include('../include/menu.php'); ?>
            ...
            <div id="container" style="width:70%; margin:15px auto;">
                <form id="InserisciBsProdotto" name="InserisciBsProdotto" method="post" action="carica_prodotto_bs2.php" enctype="multipart/form-data">
                    <table width="100%">
                        <tr>
                        ...
                        <tr>
                            <td class="cella1center" colspan="2">
                                <select name="Prodotto" id="Prodotto">
                                    <option value="<?php echo $IdProdotto ?>" selected="<?php echo $IdProdotto ?>"><?php echo $DefaultProdotto ?></option>
                                    <?php
                                    while ($row = mysql_fetch_array($sqlProdotti)) {
                                        ?>
                                        <option value="<?php echo $row['id_prodotto'] ?>"><?php echo $row['cod_prodotto'] . "  " . $row['nome_prodotto'] ?></option>
                                    <?php } ?>
                                </select>
                            </td>
                        </tr> 
                        <tr>
                            ...
                            <td class="cella1"><input type="text" name="Classificazione"  value="<?php echo $Classificazione ?>"/></td>
                        </tr> 
                        <tr>
    ...
    for ($a = 0; $a < count($arrayAziendeScrivibili); $a++) {
        ...
        if ($idAz != $_SESSION['id_azienda']) {
            ?>
                        ...
                        <tr>
                            <td class="cella2" ><?php echo $filtroSchedaTecnica2 ?></td>
                            <td class="cella1"><input style="width:90%" type="text" name="LinkSchedaTecnica"  value="<?php echo $SchedaTecnica ?>"/></td>
                        </tr>
