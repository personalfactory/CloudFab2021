...
<?php
if($_POST['key']==""){
	echo 'Nessun risultato! <a href="gestione_lab_formula.php">Torna alle Formule</a>';
	}
if(isset($_POST['key'])&&($_POST['key']!="")/*&&(preg_match("/^[a-z0-9]+$/i", $_POST['key']))*/)
 {
 	...
 	if($_SESSION['nome_gruppo_utente']=='Amministrazione'){
		...
		$query="SELECT
				...
				INNER JOIN 
					 lab_matpri_teoria
				ON 
					lab_formula.cod_lab_formula=lab_matpri_teoria.cod_lab_formula
				INNER JOIN 
					 lab_materie_prime
				ON 
					lab_matpri_teoria.id_mat=lab_materie_prime.id_mat
				WHERE 
						(lab_formula.cod_lab_formula 		LIKE '%" . $key . "%') 
	...
	//Se l'utente che effettua la ricerca NON appartiene al gruppo Amministrazione ha accesso a solo alle proprie formule	
	$query="SELECT
					lab_formula.cod_lab_formula,
					lab_formula.dt_lab_formula,
					lab_formula.utente,
	...
	if($trovati > 0)
	{
		...
		?>
        <a href="gestione_lab_formula.php">Torna alle formule</a>
        <?php include('./moduli/visualizza_lab_formula.php'); ?>
</body>
</html>
<?php
