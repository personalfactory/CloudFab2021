...
//                                                       echo "</br>numRisText:".$numRisText;
                                                            if (strlen($rowRisCar['valore_caratteristica']) > $numCarVis) {
                                                                $TrePunti = $filtroPuntini;
                                                            }
                                                            ?>
                                                            <input type="checkbox" name="RisTxt<?php echo($numRisText); ?>[]" value="<?php echo $rowRisCar['id_esperimento'] . ";" . $rowRisCar['id_carat'] . ";" . $rowRisCar['valore_caratteristica'] . ";" . $rowRisCar['note'] ?>" title="INCLUDI NEL MERGE"/>
                                                            <?php
                                                            $numRisText++;
                                                        }
                                                        echo substr($rowRisCar['valore_caratteristica'], 0, $numCarVis) . $rowRisCar['uni_mis_car'] . " " . $TrePunti
                                                        ?></td> 
                                                        <?php if ($rowRisCar['valore_dimensione'] != 0) { ?>
                                                        <td  title="<?php echo $filtroLabProvaBreve . " " . $arrayEsperimenti[$j][2] . " " . $rowListaCar['note'] ?>"><?php echo $rowRisCar['valore_dimensione'] . $rowRisCar['uni_mis_dim'] ?></td> 
                                                    <?php } ?>
                                                    <!--</tr>-->
                                                    <?php
