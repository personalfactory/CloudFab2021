...
<html>
    ...
    <div id="labContainer">
        ...
        <script language="javascript">
            function Carica() {
                document.forms["CreaFormulaDaProva"].action = "carica_lab_formula_da_prova2.php";
                document.forms["CreaFormulaDaProva"].submit();
            }
            function AggiornaCalcoli() {
                document.forms["CreaFormulaDaProva"].action = "carica_lab_formula_da_prova.php";
                document.forms["CreaFormulaDaProva"].submit();
            }
        ...
        //Recupero le aziende scrivibili per la tabella formula perchè si sta creando una nuova formula
        ...
        $sqlMatPriAll = selectAllLabMatByFiltri($_SESSION['Filtro'], $_SESSION['TipoMat'], $_SESSION['CodMat'], $_SESSION['DescriMat'], $strUtentiAziendeMatPri);
        $sqlTipoAll = findAllTipiMatPrime($strUtentiAziendeMatPri);
        $sqlParAll = findAllParametriVisByNomeAndTipo($_SESSION['NomePar'], $_SESSION['TipoPar'], $_SESSION['FiltroPar'], "id_par", $strUtentiAziende);
        commit();
...
//il form di inserimento con i valori recuperati dalla prova di laboratorio
        if ((!isset($_POST['CodiceFormula']) || $_POST['CodiceFormula'] == "") && (isset($_GET['IdEsperimento']) && isSet($_GET['CodiceFormulaOld']))) {

//La variabile CodiceFormulaOld si riferisce al codice della formula vecchia 
//ovvero la formula dalla quale è stata fatta la prova
...
//che verra salvata
            ...
            //##################################################################
            //############# QUERY AL DB ESPERIMENTO ############################
            //##################################################################

            begin();
            //NEW
            $sqlFormulaOld = findFormulaByCodice($CodiceFormulaOld);      
            $sqlEsper = findEsperimentoById($IdEsperimento);
            $sqlMatPrimeEsp = findQtaEsperimento($IdEsperimento);
            $sqlParEsp = findRisultatiParByIdEsperimento($IdEsperimento);
            commit();
            ...
            $NMtComp = 1;
            while ($rowMtComp = mysql_fetch_array($sqlMatPrimeEsp)) {

                //Serve per il calcolo delle quantita in percentuale
                $TotaleQta = $TotaleQta + $rowMtComp['qta_reale'];
                //Serve per il calcolo del costo in percentuale
                $CostoTotalePerc = $CostoTotalePerc + ($rowMtComp['qta_reale'] * $rowMtComp['prezzo'] / $fatConvKgGrammi);
...
//                echo "########### k: " . $k;
            ...
            $NMatPri = 0;
            if (mysql_num_rows($sqlMatPrimeEsp))
                mysql_data_seek($sqlMatPrimeEsp, 0);
            while ($rowMatPrime = mysql_fetch_array($sqlMatPrimeEsp)) {
                $NMatPri = $rowMatPrime['id_mat'];

                $QuantitaMatPrima = ($rowMatPrime['qta_reale']*102000)/$TotaleQta;
...
//                echo "########### p: " . $p;
            ...
            $NPar = 0;
            if (mysql_num_rows($sqlParEsp))
                mysql_data_seek($sqlParEsp, 0);
            while ($rowPar = mysql_fetch_array($sqlParEsp)) {
                $ValoreReale = 0;
                $ValoreRealePerc = 0;
                ...
                if ($rowPar['tipo'] == $PercentualeNO) {
                    ...
                    $ValoreReale = number_format(($rowPar['valore_reale'] * $TotaleQta) / 100, $PrecisioneQta, '.', '');
                    $ValoreRealePerc = number_format($rowPar['valore_reale'], $PrecisioneQta, '.', '');
                }

                //##### SALVO I PARAMETRI NELL' OGGETTO ########################               
                //Salvo le quantità relative alla vecchia formula nell' oggetto
                $_SESSION['LabParTeoria'][$p] = new LabParTeoria($CodiceFormulaOld, $rowPar['nome_parametro'], $rowPar['id_par'], $rowPar['unita_misura'], $rowPar['tipo'], $ValoreReale, $ValoreRealePerc);
                $p++;
            }//End While Parametri
//            echo "</br>##### p :" . $p;
//            for ($j = 0; $j < $p; $j++) {
...
//            }
            ...
            <body onLoad="<?php echo $actionOnLoad ?>">
                <div id="container" style="width:95%; margin:15px auto;">
                    <form id = "CreaFormulaDaProva" name = "CreaFormulaDaProva" method = "POST">
...
//        echo "oggetto pieno count: " . count($_SESSION['LabParTeoria']) 
                        ...
                        <table width="100%">
                            ...
                            <tr>
                                ...
                                <td class="cella4" colspan="3" >                            
                                    <select  name="TipoMat" id="TipoMat"  onChange="javascript:AggiornaCalcoli();" title="<?php echo $titleLabSelectTipoMat ?>">
                                        ...
                                        <?php
                                        while ($rowTipo = mysql_fetch_array($sqlTipoAll)) {
                                            ?>
                            ...
                            $NMatPri = 0;
                            while ($rowMatPri = mysql_fetch_array($sqlMatPriAll)) {
                        ...
                        <table width="100%">
                            ...
                            <?php
                            $NPar = 0;
                            while ($rowPar = mysql_fetch_array($sqlParAll)) {
                                $NPar = $rowPar['id_par'];
...
//##############################################################################  
                ...
                $_SESSION['Filtro'] = "descri_materia";
...
//    }
                ...
                $messaggioQtaMatPri = "";
                $NumErroreMt = 0;
                if (mysql_num_rows($sqlMatPriAll) > 0)
                    mysql_data_seek($sqlMatPriAll, 0);
                while ($rowMatPrime = mysql_fetch_array($sqlMatPriAll)) {
...
//##############################################################################
                ...
                $messaggioQtaPar = "";
                $NumErrorePar = 0;
                if (mysql_num_rows($sqlParAll))
                    mysql_data_seek($sqlParAll, 0);
                while ($rowPar = mysql_fetch_array($sqlParAll)) {
                    $NPar = $rowPar['id_par'];
                            ...
                            <tr>
                                ...
                                <td class="cella4" colspan="3">
                                    <select style="width:50%" name="TipoMat" id="TipoMat"  onChange="javascript:AggiornaCalcoli();" title="<?php echo $titleLabSelectTipoMat ?>">
                                        <option value="<?php echo $_SESSION['TipoMat'] ?>" selected="<?php echo $_SESSION['TipoMat'] ?>"><?php echo $_SESSION['TipoMat'] ?></option>
                                        <?php
                                        if (mysql_num_rows($sqlTipoAll))
                                            mysql_data_seek($sqlTipoAll, 0);
                                        while ($rowTip = mysql_fetch_array($sqlTipoAll)) {
                                            ?>
                        ...
                        <table width="100%">
                            ...
                            while ($rowPar = mysql_fetch_array($sqlParAll)) {
                                ...
                                <tr>
                                    ...
                                    <td class="<?php echo $nomeClassePar ?>"><?php echo($rowPar['tipo']); ?></td>
                                    <td class="<?php echo $nomeClassePar ?>"><?php echo ($rowPar['unita_misura']); ?></td>
                                    <td class="<?php echo $nomeClassePar ?>"><input type="text" name="QtaPar<?php echo($NPar); ?>" id="QtaPar<?php echo($NPar); ?>" value="<?php echo $_POST['QtaPar' . $NPar]; ?>" /></td>
