...
function findAllCarByArrayEsperimenti($stringaEsperimenti) {
    $sqlString = "SELECT * FROM serverdb.lab_risultato_car r
			JOIN serverdb.lab_esperimento e
			ON 
                            r.id_esperimento = e.id_esperimento
			
                        JOIN serverdb.lab_caratteristica c
			ON 
        		    c.id_carat = r.id_carat
			WHERE 
                            e.id_esperimento IN " . $stringaEsperimenti . "                                           
			GROUP BY c.id_carat
			ORDER BY c.caratteristica";

    $sql = mysql_query($sqlString) or die("ERROR IN script_lab_risultato_car - FUNCTION findAllCarByArrayEsperimenti -  " . $sqlString . " - " . mysql_error());
    return $sql;
}

function findQtaByProvaIdCar($idEsperimento, $idCar) {

    $stringSql = "SELECT * FROM serverdb.lab_risultato_car r
                       JOIN serverdb.lab_caratteristica c ON c.id_carat=r.id_carat
                        WHERE
                             r.id_esperimento=" . $idEsperimento . "
                             AND
                             r.id_carat =" . $idCar;
    $sql = mysql_query($stringSql)
            or die("ERROR IN script_lab_risultato_car- FUNCTION findQtaByProvaIdCar - " . $stringSql . " - " . mysql_error());
    return $sql;
}

/**
 * Data una caratteristica e un insieme di esperimenti trova il numero massimo di risultati
 * @param type $stringaEsperimenti
 * @param type $idCar
 * @return type
 */
function findMaxNumRigheRisIdCar($stringaEsperimenti, $idCar) {



    $stringSql = "SELECT MAX(num_ris) AS max_ris FROM
                (
                    SELECT count(id_esperimento) as num_ris 
                        FROM serverdb.lab_risultato_car r
                        JOIN serverdb.lab_caratteristica c ON c.id_carat=r.id_carat
                    WHERE
                        r.id_esperimento IN " . $stringaEsperimenti . "
                        AND
                        r.id_carat =" . $idCar . "  
                    GROUP BY id_esperimento
		) AS counted";

    $sql = mysql_query($stringSql)
            or die("ERROR IN script_lab_risultato_car- FUNCTION findMaxNumRigheRisIdCar - " . $stringSql . " - " . mysql_error());
    return $sql;
}
