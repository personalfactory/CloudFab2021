            ...
            if (!isset($_POST['idMacchina']) && !isset($_GET['CaricaTutti'])) {
                ...
                <div id="container" style="width:80%; margin:15px auto; ">
                    <form id="AggiornaMacchine" name="AggiornaMacchine" method="post" action="sync_carica_verso_macchina.php">
                        <table width="100%">
                            <tr>
                                <td class="cella3" colspan="2"><?php echo $titoloPaginaInvioAgg ?></td>
                            </tr>
                            <tr>
                                <td class="cella22" ><?php echo $filtroStabilimento ?> </td>
                                <td class="cella111">
                                    <select name="idMacchina" id="idMacchina">
                                        <option value="" selected=""><?php echo $labelOptionSelectStab ?></option>
                                        <?php
                                        $sql = findAllMacchineVisAbilitate("id_macchina", $strUtentiAziende);
                                        //findAllMacchine("id_macchina");
                                        while ($row = mysql_fetch_array($sql)) {
                                            ?>
                                            <option value="<?php echo $row['id_macchina']; ?>"><?php echo $row['id_macchina'] . " - " . $row['cod_stab'] . " - " . $row['descri_stab']; ?></option>
                                        <?php } ?>
                                    </select>
                                    <input type="submit" title="<?php echo $titleInviaAggStab ?>" value="<?php echo $valueButtonInvia ?>" onClick="javascript:CaricaVersoMacchina();"/></td>
                            </tr>
                            <tr>
                                <td class="cella22" style="text-align: right " colspan="2">
                                    <input type="reset" value="<?php echo $valueButtonAnnulla ?>" onClick="javascript:history.back();"/>
                                    <input type="button" id="58" title="<?php echo $titleInviaAggTuttiStab ?>" value="<?php echo $valueButtonAggiornaTutti ?>" onClick="javascript:CaricaVersoTutti();"/></td>
                            </tr>
                        </table>
                    </form>
                </div>
